...
-- 1.2: Update orders table to use enum and remove redundant phone_number
ALTER TABLE orders 
...
-- 3.2: Recent orders view (last 3 hours priority)
...
-- 4.2: Get recent urgent orders function
...
-- 5.1: Order status transition function
...
BEGIN
  ...
  valid_transition := CASE 
    ...
    WHEN current_status = 'ready' AND new_status IN ('out_for_delivery', 'completed', 'cancelled') THEN true
    WHEN current_status = 'out_for_delivery' AND new_status IN ('completed', 'cancelled') THEN true
    ELSE false
  END;
...
-- =========================================
-- MIGRATION COMPLETE
-- Backend is now CRM-ready with:
-- ✅ Proper relationships and constraints
-- ✅ Order status enum and workflow
-- ✅ Performance indexes for analytics
-- ✅ CRM-optimized views and functions
