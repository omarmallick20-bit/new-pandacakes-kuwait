CREATE OR REPLACE FUNCTION public.validate_voucher(voucher_code_param text, customer_id_param uuid, order_amount_param numeric, country_code_param character varying DEFAULT 'qa'::character varying)
 RETURNS TABLE(is_valid boolean, discount_amount numeric, discount_percentage numeric, error_message text, voucher_id uuid)
 LANGUAGE plpgsql
...
BEGIN
    ...
    -- FIXED: Apply LOWER() to make day comparison case-insensitive
    current_day_name := LOWER(TRIM(current_day_name));
    ...
    -- Check customer-specific usage limit (use table alias to avoid ambiguity)
    ...
    IF voucher_record.happy_hour_enabled = TRUE THEN
        -- Check day restriction (now case-insensitive)
