...
serve(async (req) => {
  ...
  try {
    const TAP_SECRET_KEY = Deno.env.get('TAP_SECRET_KUWAIT_KEY')?.trim();
    if (!TAP_SECRET_KEY) {
      throw new Error('TAP_SECRET_KUWAIT_KEY not configured');
    }
    ...
    console.log('Creating Tap charge (KW) for retry:', { orderId, amount: roundedAmount, currency: cc.currency, retryCount });
    ...
    console.log('Tap charge created (KW) for retry:', { chargeId, redirectUrl });
    ...
    console.error('Retry payment error (KW):', error);
    return new Response(
